...
class ZephyrFivePillar(QCAlgorithm):
    ...
    def Rebalance(self) -> None:
        ...
        # TOTAL RETURN — used for returns, win rate, vol, sizing
        tr_history = self.History(
            all_hist_syms,
            self.max_lookback + 1,
            Resolution.Daily,
        ...
        # SPLIT ADJUSTED — used ONLY for bond momentum / duration regime
        ...
        tr_closes = tr_history["close"].unstack(0)
        price_closes = price_history["close"].unstack(0)

        # -----------------------------
        # Duration regimes (BONDS ONLY)
        # -----------------------------
        corp_bond_syms = self.GetDurationRegimeForGroup(
            price_closes, self.symbols["corp_bonds"]
        )
        treasury_bond_syms = self.GetDurationRegimeForGroup(
            price_closes, self.symbols["treasury_bonds"]
        )

        # -----------------------------
        # Sector momentum
        # -----------------------------
        sector_mom = self.ComputeMomentum(self.symbols["sectors"], tr_closes)
        top_sectors = sorted(sector_mom, key=sector_mom.get, reverse=True)[:self.sector_count]

        # -----------------------------
        # Nasdaq-100 Top Stocks
        # -----------------------------
        stock_mom = self.ComputeMomentum(list(self.stock_symbols), tr_closes)
        ...
        for g, syms in risk_groups.items():
            eligible = [s for s in syms if s in tr_closes.columns and self.PassesTrend(s)]
            if not eligible:
                continue

            g_rets = tr_closes[eligible].pct_change().mean(axis=1).dropna()
            if len(g_rets) < self.winrate_lookback:
                continue

            p_win = float(np.mean(g_rets.tail(self.winrate_lookback) > 0))
            group_mom = self.ComputeGroupMomentum(eligible, tr_closes)
